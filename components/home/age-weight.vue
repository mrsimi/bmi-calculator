<template>
  <section class="grid grid-cols-2 gap-2 mb-6 text-center mt-4">
    <div class="rounded-md shadow-md bg-gray-800 p-4 w-full">
      ...
      <div class="flex items-center justify-around">
        <input min="0" max="200" type="hidden" v-model="weight" />
        <button
    ...
    <div class="rounded-md shadow-md bg-gray-800 p-4 w-full">
      ...
      <p class="text-5xl font-bold">{{age}}</p>
      <div class="flex items-center justify-around">
        <input min="0" max="100" type="hidden" v-model="age" />
        <button
          @click="updateAge(-1)"
...
export default {
  ...
  methods: {
    updateAge(val) {
      this.age += val;
      this.$emit("ageweightListener", { age: this.age, weight: this.weight });
    },
    updateWeight(val) {
      this.weight += val;
      this.$emit("ageweightListener", { age: this.age, weight: this.weight });
